...
class QuantOrchestrator:
    ...
    def run_feature_engineering(self):
        ...
        for ticker in all_tickers:
            # 1. Fetch raw data
            df = self.yf_client.get_ohlcv(ticker)
            alt_df = self.alt_client.get_merged_alt_data(ticker)
            
            # 2. Fetch Scraped Data
            scraped_df = self.repo.con.execute("SELECT date, sentiment_score FROM alt_scraped_sentiment WHERE ticker = ?", [ticker]).df()
